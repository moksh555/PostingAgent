...
Reference of all commands to run the runnable files in the Backend/ project.

Run every command from the `Backend/` directory, e.g.:
    cd /Users/moksh/Desktop/MarketingAgent/Backend
...
- Files that do `from configurations.config import ...` or `from app...`
  must be executed with `Backend/` as the working directory, either as a
  module (`python -m ...`) or as a script (`python path/to/file.py`) when
  the script itself adds Backend/ to `sys.path` (the files under `tests/`
  already do this).
...
# =============================================================================
# Starts the HTTP server defined in Backend/main.py (app = FastAPI(...)).
#
...
#   Has an `if __name__ == "__main__"` block that uploads Backend/graph.png
...
# =============================================================================
# Each of these scripts prepends Backend/ to sys.path, so they can be
