...
class TestGenerateAccessTokenFromRefreshToken:
    ...
    def test_expired_refresh_token_raises_not_authorized(self, auth_service):
        refresh_token = auth_service._encodeRefreshToken(_claims(), timedelta(seconds=-1))
...
class TestGetUserFromAccessToken:
    ...
    def test_invalid_access_token_does_not_attempt_refresh(
        ...
        monkeypatch.setattr(auth_service, "generateAccessTokenFromRefreshToken", refresh_spy)
        wrong_secret_token = auth_service._encodeRefreshToken(_claims(), timedelta(minutes=5))
